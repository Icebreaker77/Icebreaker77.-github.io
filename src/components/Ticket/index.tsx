...
const Ticket: React.FC<TicketProps> = ({price, transfers, company, routes}) => (
  <div className="ticket">
    <div className="ticket__header">
      <div className="ticket__header-price">{new Intl.NumberFormat('ru-RU').format(price)} $</div>
      ...
      {routes.map((route, index) => (
        <div className="ticket__flight" key={index}>
          <div className="ticket__flight-segment">
            <span className="ticket__flight-title ticket__route">{route.from} - {route.to}</span>
            <span className="ticket__time">{formatTime(route.departure)} - {formatTime(route.arrival)}</span>
          </div>
          <div className="ticket__flight-segment">
